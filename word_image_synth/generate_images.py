...
def generate_word(word, labels_dict, output_dir_images):
    ...
    distorsion_type = settings.get_distorsion_type()
    ...
    orientation = settings.get_orientation()
...
def generate_images_from_words(
    ...
    for i, word in enumerate(words):
        ...
        logging.info(
            f"{i + 1}/{len(words)}. Generating {num_images_per_word} "
            f"images for word: {word}"
        )
    ...
    logging.info("Finished processing. Total words processed: " 
                 f"{words_processed}")
